					...
					<c:otherwise>
						...
						<c:forEach var="dto" items="${list}">
							...
							<tr>
								...
								<td><img src="/contents/storage/jeans1.jpg"
									class="img-rounded" width="100px" height="100px"></td>
								<td><c:if test="${empty dto.size}">
										<a href="javascript:read('1')">가방</a>
									</c:if> <c:if test="${not empty dto.size}">
										<a href="javascript:read('1')">찢어진 청바지(size : ${dto.size})</a>
									</c:if></td>
								<td><input type='number' value="${dto.count}" min="1"
									max="10"></td>
								<td>50000</td>
								<td><a href="./delete/1"> <span
										class="glyphicon glyphicon-trash"></span>
								</a></td>
							</tr>
							<tfoot>
								<tr style="background-color: beige; font-size: large">
									<th colspan="6" style="padding: 30px;">주문금액 150,000원 + 배송비
										3,000원 = 153,000 <a href="/order/create"> <img
											src="/svg/bag-heart-fill.svg" title="주문하기"
											style='width: 30px; margin-left: 30px'></a> <a
										href="/contents/mainlist/1"> <img
											src="/svg/box2-heart.svg" title="쇼핑계속"
